...
def problem0c(circle, n, window):
    ...
    window.render(0.5)
    xvalue = circle.center.x + (2 * circle.radius)
    for k in range (n):
        circle1 = rg.Circle(rg.Point(xvalue, circle.center.y), circle.radius)
        circle1.attach_to(window)
        xvalue = xvalue + (2 * circle.radius)
        window.render(0.5)

...
Authors: David Mutchler, Vibha Alangar, Matt Boutell, Dave Fisher,
         ...
         their colleagues, and Tierney Greenhoe.
...
def problem2b(rect, n, delta, win):
    rect.attach_to(win)
    win.render()
    up_right = rg.Point(rect.get_upper_right_corner().x + delta, rect.get_upper_right_corner().y - delta)
    bottom_left = rg.Point(rect.get_lower_left_corner().x - delta, rect.get_lower_left_corner().y + delta)
    for k in range(n - 1):
        rectangle = rg.Rectangle(up_right, bottom_left)
        rectangle.attach_to(win)
        up_right.x = up_right.x + delta
        up_right.y = up_right.y - delta
        bottom_left.x -= delta
        bottom_left.y += delta
    win.render()
    """
